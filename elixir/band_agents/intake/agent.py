"""elixire-intake band remote agent — full patient intake conversation + document processing."""
...
logger = logging.getLogger("elixire.intake")
    ...
    while True:
        ...
        try:
            adapter = DeliveringLangGraphAdapter(
                ...
                additional_tools=[run_intake_conversation, run_document_processor],
                custom_section=load_role_prompt(AGENT_DIR),
            )
            agent = create_remote_agent("intake", adapter, AGENT_DIR)
            logger.info("starting Elixire-Intake (attempt %d)…", attempt)
            await agent.run()
            wait = backoff + random.uniform(0, 20)
            logger.warning("Intake disconnected gracefully (attempt %d) — reconnecting in %.0fs", attempt, wait)
            ...
            if "already_connected" in msg or "429" in msg or "rate" in msg:
                ...
                logger.error("Intake fatal error (attempt %d): %s", attempt, exc)
                raise
